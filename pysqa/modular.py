...
class ModularQueueAdapter(BasisQueueAdapter):
    def __init__(self, config, directory='~/.queues'):
        ...
        self._queue_to_cluster_dict = {k: v["cluster"] for k, v in self._config["queues"].items()}
    ...
    def submit_job(self, queue=None, job_name=None, working_directory=None, cores=None, memory_max=None,
        ...
        commands = ["module --quiet swap cluster/{};".format(cluster_module)] + \
            self._commands.submit_job_command + \
            [queue_script_path]
        out = self._execute_command(commands=commands,
                                    working_directory=working_directory,
                                    split_output=False)
        if out is not None:
    ...
    def enable_reservation(self, process_id):
        ...
        Returns:
            str:
        """
        cluster_module, cluster_queue_id = self._resolve_queue_id(process_id=process_id,
                                                                  cluster_dict=self._config["cluster"])
        cluster_commands = self._switch_cluster_command(cluster_module=cluster_module)
        commands = cluster_commands + self._commands.enable_reservation_command + [str(cluster_queue_id)]
        out = self._execute_command(commands=commands,
                                    split_output=True)
        if out is not None:
            return out[0]
    ...
    def delete_job(self, process_id):
        ...
        cluster_module, cluster_queue_id = self._resolve_queue_id(process_id=process_id,
                                                                  cluster_dict=self._config["cluster"])
        cluster_commands = self._switch_cluster_command(cluster_module=cluster_module)
        commands = cluster_commands + self._commands.delete_job_command + [str(cluster_queue_id)]
        out = self._execute_command(commands=commands,
                                    split_output=True)
        if out is not None:
            return out[0]
    ...
    def get_queue_status(self, user=None):
        ...
        for cluster_module in self._config["cluster"]:
            cluster_commands = self._switch_cluster_command(cluster_module=cluster_module)
            out = self._execute_command(commands=cluster_commands + self._commands.get_queue_status_command,
                                        split_output=False)
