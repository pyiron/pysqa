...
* `pip install pysqa` - base version - with minimal requirements only depends on `jinja2`, `pandas` and `pyyaml`.
* `pip install pysqa[sge]` - sun grid engine (SGE) version - in addition to the base dependencies this installs 
  `defusedxml` which is required to parse the `xml` files from `qstat`. 
* `pip install pysqa[remote]` - remote version - in addition, to the base dependencies this installs `paramiko` and 
